...
class Model:
    ...
    def __init__(self) -> None:
        self.reader = open(logger_file, "rb")
        self.readOffset = 0
...
class HugeFileViewer(QWidget, Ui_HugeFileViewer):
    ...
    def __init__(self, parent: QWidget = None) -> None:
        ...
        self.clear_signal.connect(self.init)

        self.textEdit.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.textEdit.customContextMenuRequested.connect(self.customContextMenuRequested.emit)
    ...
    def refresh(self, tail: bool=F):
        if not self.lines:
            return
        if tail or self.ptr.row > self.lines - 1:
            ...
            self.ptr.col = max(0, min(self.ptr.col, self.lineLen(self.ptr.row) - 1))
    ...
    def _trigger_selection(self, e:QKeyEvent):
